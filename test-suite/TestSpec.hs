...
spec = do
    ...
    describe "wide string" $ do
        ...
        describe "dropLeftNoPad" $ do
            ...
            it "records extra padding needed" $ dropLeftNoPad 1 (WideString "㐀㐁") `shouldBe` (1, WideString "㐁")
        describe "dropRightNoPad" $ do
            prop "agrees with dropRight" $ propDropRightNoPad WideString
            it "records extra padding needed" $ dropRightNoPad 1 (WideString "㐀㐁") `shouldBe` (WideString "㐀", 1)
        describe "dropBothNoPad" $ do
            it "records extra padding needed"      $ dropBothNoPad 1 1 (WideString "a㐀㐁") `shouldBe` (0, WideString "㐀", 1)
            it "drops less on the right if it can" $ dropBothNoPad 1 1 (WideString "㐀㐁") `shouldBe` (0, WideString "㐁", 0)
